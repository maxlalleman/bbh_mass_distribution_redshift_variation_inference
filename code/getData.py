...
def initialize_ar_lnm1(all_lnm1_samples):

    initial = {
        'logR20':np.log10(np.exp(0.6)),
        'ar_lnm1_std':1.5,
        'ar_lnm1_tau':0.75,
        'ar_lnm1_mu':0
        }

    m1_samples = np.exp(all_lnm1_samples) 

    p_m1 = np.zeros_like(m1_samples)
    p_m1 += (1.-0.2)*0.9*np.exp(-(m1_samples-10.)**2/2.)/np.sqrt(2.*np.pi)
    p_m1 += (1.-0.2)*0.1*np.exp(-(m1_samples-35.)**2/(2.*4.**2.))/np.sqrt(2.*np.pi*4.**2)
    p_m1_pl = np.ones(m1_samples.size)
    p_m1_pl[m1_samples<10.] = 1.
    p_m1_pl[m1_samples>=10.] = (m1_samples[m1_samples>=10.]/10.)**(-2.5)
    p_m1_pl /= (8.+10.**(2.-2.*2.5) - 10.**(1.-2.5) - 8.*2.5)/(1.-2.5)
    p_m1 += 0.2*p_m1_pl 

    ind_20 = np.argmin((m1_samples-20.)**2)
    ln_dN_dlnm1 = np.log(p_m1*m1_samples)
    ln_dN_dlnm1 *= np.log(10.**initial['logR20'])/ln_dN_dlnm1[ind_20]
    
    ws = np.diff(ln_dN_dlnm1[ind_20:]) 
    deltas_high = np.diff(all_lnm1_samples[ind_20:])
    ws_unscaled_high = ws/initial['ar_lnm1_std']/np.sqrt(1.-np.exp(-2.*deltas_high/initial['ar_lnm1_tau']))

    ws_low = np.diff(ln_dN_dlnm1[:ind_20])[::-1]
    deltas_low = np.diff(all_lnm1_samples[:ind_20])[::-1]
    ws_unscaled_low = ws_low/initial['ar_lnm1_std']/np.sqrt(1.-np.exp(-2.*deltas_low/initial['ar_lnm1_tau']))
    ws_unscaled = np.concatenate([ws_unscaled_low[::-1],ws_unscaled_high])
...
if __name__=="__main__":
    ...
    #test = getSamples(reweight=True,weighting_function=reweighting_function_arlnm1_q)
